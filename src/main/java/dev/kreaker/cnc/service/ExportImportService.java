...
public class ExportImportService {
      ...
      try (CSVWriter writer = new CSVWriter(new OutputStreamWriter(outputStream))) {
         ...
         for (CatalogItemDTO item : items) {
            String[] row = {item.getModulo(), item.getCampo(), item.getValor(),
                     String.valueOf(item.getSbsNo()), item.getDescripcion(),
  ...
  /**
   ...
   private CatalogImportRow parseCatalogRow(String[] row) {
      ...
      Integer orden =
               row.length > 5 && !row[5].trim().isEmpty() ? parseInteger(row[5].trim(), "Orden")
                        : null;
      String domain = row.length > 6 && !row[6].trim().isEmpty() ? row[6].trim() : null;
      Integer status =
               row.length > 7 && !row[7].trim().isEmpty() ? parseInteger(row[7].trim(), "Status")
                        : 1;
    ...
    */
   private void validateCatalogImportRow(String modulo, String campo, String valor,
            Integer cadena) {
