...
public class UserRepository {
   ...
   private static final RowMapper<CncUser> USER_ROW_MAPPER = (rs, rowNum) -> CncUser.builder()
            .id(rs.getLong("id")).username(rs.getString("username")).email(rs.getString("email"))
            .password(rs.getString("password")).displayName(rs.getString("display_name"))
            .enabled(rs.getInt("enabled") == 1).createdAt(rs.getString("created_at"))
   ...
   public Optional<CncUser> findByUsername(String username) {
      var users = jdbcTemplate.query("SELECT * FROM users WHERE username = ?", USER_ROW_MAPPER,
               username);
